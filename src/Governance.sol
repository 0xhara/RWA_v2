...
contract Governance is Initializable, Ownable {
    ...
    function createProposal(uint256 assetId, string memory description, ProposalType proposalType) external {
    ...
    function executeProposal(uint256 proposalId) external {
        ...
        uint256 totalShares = assetListing.assets(proposal.assetId).valuation;
        ...
        executeProposalAction(proposal.assetId, proposal.proposalType);

        emit ProposalExecuted(proposalId, true);
    }

    function executeProposalAction(uint256 assetId, ProposalType proposalType) internal {
        if (proposalType == ProposalType.SellAsset) {
            // Logic to sell the asset through the AssetListing contract
            assetListing.sellAsset(assetId);
        } else if (proposalType == ProposalType.DistributeFunds) {
            // Logic to distribute funds to fractional owners
            assetListing.distributeFunds(assetId);
        }
